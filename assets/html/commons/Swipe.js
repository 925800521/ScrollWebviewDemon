function Swipe(container, options) {
    ...
    var browser = {
        ...
        transitions: function(temp) {
            ...
            for (var i in props) if (void 0 !== temp.style[props[i]]) return true;
            return false;
    ...
    var events = {
        ...
        move: function(event) {
            if (event.touches.length > 1 || event.scale && 1 !== event.scale) return;
            ...
            if ("undefined" == typeof isScrolling) isScrolling = !!(isScrolling || Math.abs(delta.x) < Math.abs(delta.y));
            if (!isScrolling) {
            ...
            if (!isScrolling) if (isValidSlide && !isPastBounds) {
                if (direction) {
                    if (options.continuous) {
                        move(circle(index - 1), -width, 0);
